...
async function getStaticPaths() {
    const client = await external_mongodb_.MongoClient.connect("mongodb+srv://rlawnsqja805:mzGU0xMM5ubotsQr@cluster0.petdo.mongodb.net/meetups?retryWrites=true&w=majority");
    const db = client.db();
    const meetupdsCollection = db.collection("meetups");
    const meetups = await meetupdsCollection.find({}, {
        _id: 1
    }).toArray();
    client.close();
    return {
...
async function getStaticProps(context) {
    ...
    const db = client.db();
    const meetupdsCollection = db.collection("meetups");
    const selectedMeetup = await meetupdsCollection.findOne({
